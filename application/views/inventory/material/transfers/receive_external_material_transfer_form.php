                ...
                <div class="col-xs-12">
                    ...
                    <div class="col-xs-12 table-responsive">
                        <table class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    <th>Material/Asset</th><th>Quantity</th><th>Unit</th><th>Remarks</th><td></td>
                                </tr>
                            </thead>
                            <tbody>
                            <?php
                                $items = $transfer->material_items();
                            ...
                            ?>
                            <?php
                            $items = $transfer->asset_items();
                            foreach($items as $item){
                                $asset = $item->asset_sub_location_history()->asset();
                                ?>
                                <tr>
                                    <td style="width: 35%">
                                        <?= wordwrap($asset->asset_code(), 100, '<br/>') ?>
                                        <input type="hidden" name="item_type" value="asset">
                                    </td>
                                    <td></td>
                                    <td></td>
                                    <td style="width: 40%"><textarea rows="1" class="form-control" name="remarks"></textarea></td>
                                    <td>
                                        <input name="asset_id" type="checkbox" value="<?= $asset->{$asset::DB_TABLE_PK} ?>" >
                                    </td>
                                </tr>
                                <?php
                            }
