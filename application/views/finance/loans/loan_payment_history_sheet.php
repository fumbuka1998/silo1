...
<table width="100%" border="1" cellspacing="0" style="font-size: 11px">
    ...
    if($employee_loan_payments){
         $ballance = $employee_loan->total_loan_amount;
        foreach ($employee_loan_payments as $payment) {
           $ballance = $ballance - $payment->amount;
            ?>
            <tr>
                <td style="text-align: center"><?= $count ?></td>
                <td><?= set_date($payment->receipt_date) ?></td>
                <td style="text-align: right"><?= number_format($payment->amount,2) ?></td>
                <td style="text-align: right"><?= number_format($ballance,2) ?></td>
            </tr>
            <?php
            $count++;
        }
