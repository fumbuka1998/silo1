...
<div style="width: 100%">
    <div class="btn-group">
        ...
        <ul class="dropdown-menu" role="menu">
            <?php
            $can_retire = ($imprest_voucher->handler_id == $this->session->userdata('employee_id'))  || check_permission('Administrative Actions');
    ...
    <div id="retire_imprest_<?= $imprest_voucher->{$imprest_voucher::DB_TABLE_PK} ?>" class="modal fade imprest_retirement_form" role="dialog">
        <?php
            $this->load->view('finance/approved_cash_requisitions/imprest/imprest_retirement_form');
        ?>
    </div>
    <div id="imprest_documents_<?= $imprest_voucher->{$imprest_voucher::DB_TABLE_PK} ?>" class="modal fade imprest_ducuments" role="dialog">
        <?php
            $this->load->view('finance/approved_cash_requisitions/imprest/imprest_ducuments');
        ?>
    </div>
</div>