    ...
    <span>
        <div class="btn-group">
            ...
            <ul class="dropdown-menu" role="menu">
                ...
                ?>
                        <li>
                            <button type="button" style="margin-left: 1px; text-align: left; color: #777777; padding: 3px 20px;" modal-head="Requisition Form" button-link="<?= base_url('requisitions/edit_requisition_form/' . $requisition->requisition_id) ?>" title="Edit <?= $requisition->requisition_number() ?>" data-toggle="modal" id="edit_requisition_<?= $requisition->requisition_id ?>" data-target="#edit_o_approve_requisition_<?= $requisition->requisition_id ?>" class="openRequisitionModal btn btn-block btn-default btn-xs"><i class="fa fa-edit"></i> Edit</button>
                        </li>
                        <li>
                    ...
                    if ($forwarded_to_employee && !$special_level_approval) {
                        $can_approve = $requisition->status != 'INCOMPLETE' && $current_approval_level && $this->session->userdata('employee_id') == $requisition->forwarded_to_employee_approval(true);
                    } else if ($can_override_prev && !$special_level_approval) {
                        $can_approve = $requisition->status != 'INCOMPLETE' && $requisition->status != 'APPROVED' && ($requisition->foward_to == null);
                        ...
                        $can_approve = $requisition->status != 'INCOMPLETE' && $current_approval_level && in_array($this->session->userdata('employee_id'), $current_approval_level->can_approve_positions()) && ($last_approval ? $last_approval->forward_to == null : $requisition->foward_to == null);
                    }

                    if ($can_approve) {
                    ?>
                        <li>
                            <button type="button" style="margin-left: 1px; text-align: left; color: white; padding: 3px 20px;" modal-head="Requisition Approval Form" button-link="<?= base_url('requisitions/approve_requisition_form/' . $requisition->requisition_id) ?>" title="Approve <?= $requisition->requisition_number() ?>" data-toggle="modal" id="approve_requisition_<?= $requisition->requisition_id ?>" data-target="#edit_o_approve_requisition_<?= $requisition->requisition_id ?>" class="openRequisitionModal btn btn-block btn-success btn-xs"><i class="fa fa-check-square-o"></i> Act</button>
                        </li>
                <?php
                    }
                }
        ...
        ?>
                <div rerquisition_id="<?= $requisition->requisition_id ?>" id="edit_o_approve_requisition_<?= $requisition->requisition_id ?>" class="modal fade <?= $can_approve ? 'approve_requisition_form' : 'edit_requisition_form' ?>" role="dialog">
...
<script>
    ...
    function initialize_requisition_form(modal) {
        ...
        var calculate_vat_amount = function() {
            ...
            var vat_amount = (parseFloat(vat_percentage) / 100) * (overall_total_amount);
    ...
    $(document).ready(function() {
        $(".openRequisitionModal").off('click').on("click", function() {
            start_spinner();
            let modal = $(this).closest('span').find('.modal');
