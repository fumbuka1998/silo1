...
class Equipment_requisitions extends CI_Controller{
    ...
    public function save_equipment_requisition(){
        ...
        if($requisition->save()){
            if ($edit) {
                $requisition->delete_equipment_items();
            } else {
            ...
            $this->load->model(['Requisition_equipment_item', 'requisition_cash_item']);
            ...
            foreach ($item_types as $index => $item_type){
                if($quantities[$index] > 0){
                    $requisition_item = $item_type == 'equipment' ? new Requisition_equipment_item() : new Requisition_cash_item();
                    if($item_type == 'equipment'){
                        $requisition_item->asset_group_id = $item_ids[$index];
                    ...
                    $requisition_item->rate_mode = $rate_modes[$index];
                    $requisition_item->duration = $durations[$index];
