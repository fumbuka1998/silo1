...
$('#asset_group_list').DataTable({
    ...
    "serverSide": true,
    "ajax" : {
        url: base_url + "asset_register/assets/asset_group_list",
        type: 'POST'
    },
    "columns": [
        {"orderable": true},
            ...
            if(button.attr('initialized') != 'true'){
                button.click(function () {
                    ...
                    if(group_name.trim() != ''){
                        ...
                        modal.modal('hide');
                        $.post(
                            base_url + "asset_register/Assets/save",
                            {
                                group_name:group_name,
                                group_id: group_id,
                                parent_id:parent_id,
                                ...
                                stop_spinner();
                                $('#asset_group_list').DataTable().draw('page');
                            }
                        );
                    } else {
            ...
            if(button.attr('active') != 'true') {
                button.click(function () {
                    if(confirm('Are you sure?')){
                        start_spinner();
                        $.post(
                            base_url + "asset_register/Assets/delete_asset_group",
                            {
                                group_id: button.attr('group_id')
                            }, function () {
                                $('#asset_group_list').DataTable().draw('page');
                            }
                        ).complete(function(){
...
$('.save_depreciation_rate').each(function () {
    ...
    if(button.attr('initialized') != 'true'){
        button.click(function () {
            ...
            var asset_depreciation_rate_id = modal.find("input[name='asset_depreciation_rate_id']").val();
            ...
            tbody.find('input[name="asset_group_id"]').each(function(){
                ...
                asset_group_ids[i]=row.find('input[name="asset_group_id"]');
                depreciation_rates[i]=row.find('input[name="depreciation_rate"]');
            ...
            console.log(start_date,asset_group_ids,depreciation_rates);

            if(start_date != ''){
                start_spinner();
                modal.modal('hide');
                $.post(
                    base_url + "asset_register/Asset_depreciation_rates/save",
                    {

                        asset_depreciation_rate_id:asset_depreciation_rate_id,
...
$('#equipment_requisitions_table').DataTable({
    "processing": true,
    "serverSide": true,
     "ajax": {
                url: base_url + "asset_register/Equipment_requisitions",
                type: 'POST',
     ...
     $(this).find('tr').each(function () {
         $(this).find('td:last-child').attr('nowrap', 'nowrap');
                ...
                //save equipment requisition

                $('.save_equipment_requisition').each(function(){
                        var button = $(this);
                        if(button.attr('active') != 'true') {
                            button.click(function () {

                                save_equipment_requisition($(this));
                            });
                            button.attr('active', 'true');
                        }
                });
