...
class Payment_voucher extends MY_Model{
    ...
    public function payment_voucher_number(){
        return 'PV/'.add_leading_zeros($this->{$this::DB_TABLE_PK});
    }

	public function detailed_reference(){
    	$reference = $this->reference != '' ? $this->reference : null;
		return !is_null($reference) ? $this->payment_voucher_number().' - '.$reference : $this->payment_voucher_number();
	}
    ...
    {
        $this->load->model('payment_voucher_credit_account');
        $where = ['payment_voucher_id'=>$this->{$this::DB_TABLE_PK}];
        $credit_accounts = $this->payment_voucher_credit_account->get(0,0,$where);
        return !empty($credit_accounts) ? array_shift($credit_accounts) : false;
    }
    ...
    public function payment_vouchers_list($limit, $start, $keyword, $order,$project_id = null){
        ...
        if($for_project){
            ...
            $sql .= $confidentiality_position ? ' WHERE confidentiality_chain_position <=' .$confidentiality_position : '' ;
        }
        ...
        $where_clause = $for_project ? ' WHERE credit_account_id IN ( SELECT account_id FROM project_accounts WHERE project_id = '.$project_id.') ' : ( $confidentiality_position  ? ' WHERE confidentiality_chain_position <=' .$confidentiality_position : '' );
        if($keyword !=''){
    ...
    public function amount(){
        $sql = 'SELECT (COALESCE(SUM(amount),0) + COALESCE(SUM(vat_amount),0) + COALESCE(SUM(withheld_amount),0)) AS amount FROM payment_voucher_items
    ...
    public function payment_voucher_default_description(){
        $requistion_by_nature = $this->requisition_approval_payment_voucher();
        $payment_request_by_nature = $this->purchase_order_payment_request_approval_payment_voucher();
        $sub_contract_payment_by_nature = $this->sub_contract_payment_requisition_approval_payment_voucher();
        ...
        if($requistion_by_nature){
            $approval = $this->requisition_approval_payment_voucher()->requisition_approval();
            $request = $approval->requisition();
            $desc = 'Payment for approved cash request as per '.$request->requisition_number().' of '.$request->cost_center_name().'';
        } else if($payment_request_by_nature){
            $approval = $this->purchase_order_payment_request_approval_payment_voucher()->purchase_order_payment_request_approval();
            $request = $approval->purchase_order_payment_request();
            $desc = 'Payment for approved purchase order payment request as per '.$request->request_number().' of '.$request->cost_center_name().'';
        } else if($sub_contract_payment_by_nature) {
            $approval = $this->sub_contract_payment_requisition_approval_payment_voucher()->sub_contract_payment_requisition_approval();
            $request = $approval->sub_contract_requisition();
            $desc = 'Payment for approved sub contract payment request as per '.$request->sub_contract_requisition_number().' of '.$request->cost_center_name().'';
        } else {
            $approval = false;
            $desc = 'N/A';
        }
    ...
    public function bulk_payment_list($creditor_type =  null,$vendor_id = null,$currency_id = null){
        ...
        $invoice_where_clause = ' WHERE is_final = "1" AND purchase_order_payment_requests.status = "APPROVED" AND purchase_order_payment_request_approval_invoice_items.purchase_order_payment_request_approval_id NOT IN(
        SELECT purchase_order_payment_request_approval_id FROM payment_request_approval_journal_vouchers
        )';
